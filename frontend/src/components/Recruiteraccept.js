...
export default class Recruiteraccept extends Component {
    constructor(props) {
        super(props);
        this.state = {
            alldata: [],
    ...
    componentDidMount() {
        ...
        axios.post("http://localhost:4000/Appliedjobs/acceptedapplicants",applicants)
        .then(response => {
            if(response){
                console.log("ok");
                console.table(response.data.data);
            }
            this.setState({reqdata: response.data.data});
                ...
                <nav className="navbar navbar-expand-lg navbar-light bg-light">
                    ...
                    <div className="collapse navbar-collapse">
                        <ul className="navbar-nav mr-auto">
                            ...
                            <li className="navbar-item">
                                <Link to="/Recruiteraddjob" className="nav-link">Add jobs</Link>
                            </li>                            
                        </ul>
                    </div>
                </nav>
                            ...
                            <Table size="small">
                                <TableHead>
                                    <TableRow>
                                            ...
                                            <TableCell>
                                            Job Title
                                            <div className="font-icon-wrapper" onClick={()=>{this.onsortup("title");}}> <IconButton aria-label="up" ><ArrowUpwardIcon /></IconButton></div>
                                            <div className="font-icon-wrapper" onClick={()=>{this.onsortdown("title");}}> <IconButton aria-label="down" ><ArrowDownwardIcon /></IconButton></div> 
                                            </TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
